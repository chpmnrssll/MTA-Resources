...
function to_hex(n)
	if(type(n) ~= "number") then
		outputDebugString("non-number type passed in.")
	end
	
	-- checking not float
	if(n - math.floor(n) > 0) then
		outputDebugString("trying to apply bitwise operation on non-integer!")
	end
...
h = to_hex(d)
outputConsole(h)
outputConsole(to_dec(h))

for i = 1, 100000 do
	h = hex.to_hex(i)
	d = hex.to_dec(h)
	if(d ~= i) then 
		outputDebugString("failed " .. i .. ", " .. h)
	end
end
]]
